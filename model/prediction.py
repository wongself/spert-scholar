...
from . import util
from .reader import BaseInputReader
...
def _convert_pred_entities(entity_types: torch.tensor,
...
def _convert_pred_relations(rel_clf: torch.tensor, rels: torch.tensor,
    ...
    pred_rel_types = (rel_nonzero % rel_class_count
    ...
    for i in range(pred_rel_types.shape[0]):
        ...
        pred_head_type_idx, pred_tail_type_idx = pred_rel_entity_types[i][
            0].item(), pred_rel_entity_types[i][1].item()
...
def store_predictions(documents, pred_entities, pred_relations):
    ...
    for i, doc in enumerate(documents):
        ...
        converted_entities = sorted(converted_entities,
        ...
        for relation in sample_pred_relations:
            ...
            converted_head = dict(type=head_type,
                                  start=head_span_tokens[0].index,
                                  end=head_span_tokens[-1].index + 1)
            converted_tail = dict(type=tail_type,
        ...
        converted_relations = sorted(converted_relations,
                                     key=lambda r: r['head'])
